...
void MaximumSalsaCMSBaseline::increment(const char * str)
{
	...
	for (int row = 0; row < height; ++row) {
		...
		uint index;
		// FIXED BUG: if (row <= height_plus_1_over_2)
		if (row < height_plus_1_over_2)
		{
			//index = hashes.low64 & width_mask;
			//hashes.low64 >>= log_width;

			index = hashes.low64 % this->width;
			hashes.low64 /= this->width;	

		}
		else
		{
			//index = hashes.high64 & width_mask;
			//hashes.high64 >>= log_width;
	
				
			index = hashes.high64 % this->width;
			hashes.high64 /= this->width;
		}
#endif // USE_128_BIT_XXHASH
...
uint64_t MaximumSalsaCMSBaseline::query(const char * str)
{
	...
	uint index = hashes.low64 % this->width;
	hashes.low64 /= this->width;	
