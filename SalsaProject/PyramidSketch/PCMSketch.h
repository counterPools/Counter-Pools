...
public:
	PCMSketch(int _word_num, int _d, int word_size, int seed=42);
...
PCMSketch::PCMSketch(int _word_num, int _d, int word_size, int seed)
{
	d = _d;
	word_num = _word_num;
	...
	for(int i = 0; i < d; i++)
		bobhash[i] = new BOBHash(seed*(7 + i) + i + 100);
}
...
void PCMSketch::Insert(const char *str)
{
	...
	uint64 hash_value = (bobhash[0]->run(str, FT_SIZE));
